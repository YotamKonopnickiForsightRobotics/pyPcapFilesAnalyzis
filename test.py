...
# === Plot speed for each joint ===
for joint_label, data in joint_data.items():
    filtered_times = [t for t, h, e in zip(data["Time_ms"], data["Is_Homing"], data["Is_Engaged"]) if not h and e]
    filtered_speeds = [s for s, h, e in zip(data["Speed_deg_per_s"], data["Is_Homing"], data["Is_Engaged"]) if not h and e]
    filtered_accels = [a for a, h, e in zip(data["Acceleration_deg_per_s2"], data["Is_Homing"], data["Is_Engaged"]) if not h and e]
    ...
    # Plot Acceleration
    plt.figure(figsize=(12, 6))
    plt.plot(filtered_times, filtered_accels, label=f"{joint_label} Acceleration (deg/s²)", linewidth=1)
    plt.xlabel("Time (ms)")
    plt.ylabel("Acceleration (deg/s²)")
    plt.title(f"{joint_label} Acceleration over Time (Filtered: Engaged & Not Homing)")
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"robot_acceleration_filtered_plot_{joint_label}.png")
    plt.close()
